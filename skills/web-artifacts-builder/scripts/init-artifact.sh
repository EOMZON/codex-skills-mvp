...
# FAVICON_BASELINE
echo "🎯 Injecting favicon baseline..."
node --input-type=module - <<'NODE'
import fs from 'node:fs';
const file = 'index.html';
let html = fs.readFileSync(file, 'utf8');
const marker = '<!-- favicon-baseline -->';
if (!html.includes(marker)) {
  const encodedSvg = "%3Csvg%20xmlns%3D'http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg'%20viewBox%3D'0%200%2064%2064'%3E%3Crect%20width%3D'64'%20height%3D'64'%20rx%3D'14'%20fill%3D'%230b0b0b'%2F%3E%3Cpath%20d%3D'M26%2C20L46%2C32L26%2C44Z'%20fill%3D'%23fff'%2F%3E%3C%2Fsvg%3E";
  const pngBase64 = "iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAWUlEQVR42u3WMQoAIAhAUXfvf96ioTEqs4T8goND9AYVRVVLZAoAAAAAWB61eAroMarDAB4QWfl0BjhBuAGsCFeABXIFsIP4E5CzCUPHMOcq5h4AAADAN4AKk13GRFxvKeYAAAAASUVORK5CYII=";
  const block = `
    ${marker}
    <meta name="theme-color" content="#0b0b0b" />
    <link rel="icon" href="data:image/svg+xml,${encodedSvg}" type="image/svg+xml" />
    <link rel="icon" href="data:image/png;base64,${pngBase64}" type="image/png" sizes="32x32" />
`;
  if (html.includes("</title>")) html = html.replace(/<\/title>/, `</title>${block}`);
